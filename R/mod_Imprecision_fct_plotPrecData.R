...
plotPrecData <- function(data, x_var, y_var, qc_level, col_level = NULL, analyte, plot_dim) {
  ...
  if (!is.null(col_level) && col_level != "") {
    data <- dplyr::filter(data, !!rlang::sym(col_level) == qc_level)
  }

  p <- ggplot2::ggplot(data, ggplot2::aes_string(x = x_var, y = y_var, group = x_var))+
    ...
    ggplot2::geom_jitter(
      ...
      ,ggplot2::aes(
        text = paste0(
          "Day: ", !!rlang::sym(x_var), "\n"
          ,analyte, ": ", round(!!rlang::sym(y_var), 2)
        )
      )
    )+
  ...
  p <- plotly::ggplotly(p, tooltip = "text", width = plot_dim$output_pid_width, height = plot_dim$output_pid_height)
