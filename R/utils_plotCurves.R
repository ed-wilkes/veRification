...
plotCurves <- function(data, value, label, positive, curves_data, threshold, type, plot_height, plot_width) {
  ...
  # Plot data
  set.seed(123)
  p <- ggplot2::ggplot(dplyr::filter(curves_data, iter %in% sample(iter, size = 100)))+
    ggplot2::geom_abline(
      slope = slope
      ,intercept = intercept
      ,colour = "red2"
    ...
    ggplot2::geom_path(aes(x = x, y = y), alpha = 0.2, size = 0.5)+
    ggplot2::geom_path(data = df_curve, aes(x = x, y = y), alpha = 0.7, col = "dodgerblue3", size = 1)+
    ggplot2::geom_point(aes(x = point_x, y = point_y), size = 4, col = "dodgerblue")+
